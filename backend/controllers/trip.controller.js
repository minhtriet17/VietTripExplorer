...
export const createTrip = async (req, res, next) => {
    ...
    try {
      ...
      if (response.data.length > 0) {
        ...
        console.warn("Không tìm thấy tọa độ cho địa điểm:", location.displayName);
      }
    } catch (geoErr) {
      ...
      (trip.locations || []).map(async (location) => {
        ...
        if (!locationImage) {
          ...
          if (imageData) {
            ...
            locationImage = `https://source.unsplash.com/800x400/?${encodeURIComponent(displayName || "vietnam")}`;
          }
        }
